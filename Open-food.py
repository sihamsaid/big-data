...
# -*- coding: utf-8 -*-
"""
As part of our training at the IA School. We have to provide a Big Data project using streaming and plot real time 
charts. This script allows scrapping product information from https://fr.openfoodfacts.org/. Then it 
publishes each product's information as a Kafka message to a given topic.

This script will:

- Use Webdriver /beautifulsoup to scrap the needed information.
- Start many workers (process) that work concurrently. For that I use the multiprocess package. I split the number of pages by number of worker.
see Constant part for more details
- Each worker will send a message to 'products-events' topic.
...
Note: This script needs a python version 3. Also, ensure to install the following packages (pip install package-name)
...
To run the script in a Linux/MacOS command line execute: python3.8 Open-food.py &. `&` will executed it as background process
"""
...
def get_webdriver_configuration():
    """
    Gets the web driver configuration. It download the chrome webdriver required by operating system
...
def download_webdriver():
    """
    Downloads the web driver for the local operation system
    ...
    if path.exists(local_path):
        logging.info('Web driver already exist.')
        return
...
def get_webdriver():
    """
     Gets a webdriver instance. It downloads the webdriver for operation system running this script. 
     Note: Depending on the you operating system you might have to give the executable permissions to the downloaded chrome driver
...
def get_product_caracteristics(driver):
    ...
    Returns
    -------
        dict: the product's caracteristics value if exist, If an information is missing it will be replaced by UNKNOWN_VALUE
        caracteristics = {
...
def get_product_info(driver, url):
    ...
    info['Nom'] = get_product_name(driver)
    ...
    info['Nutri_score'] = nutri_score
    info['NOVA'] = nova
    info['Eco_Score'] = eco_score
    ...
    # Get nutritional
    nutrition = get_100g_nutritional_info(driver)
    info.update(nutrition)
...
def run_process(process_id=1, pages=[]):
    """
     Gets the products dataframe. This method will load all produts pages. Stored the products as a json and csv file. 
    ...
    for page_number in range(len(pages)):
        ...
        size = len(urls)
        for product_index in range(size):
            url = urls[product_index]
            try:
                info = get_product_info(driver=driver, url=url)
                print(f'data=>{info}')
                producer.send('products-events', info)
                total_success += 1
...
def main():
    """
     ...
     Once all workers/process have beed start, it will wait until their end.
    ...
    while index < len(buckets):
        process_id = index
        pages = buckets[index]
        process = pool.apply_async(run_process, args=(
            process_id, pages, ))
        jobs.append(process)
        index += 1
